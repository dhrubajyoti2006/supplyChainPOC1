...
import type { DiscoveryPlaceDetails, DiscoveryRequest, DiscoveryResult, DiscoveryScan } from "../types/Discovery";
...
const defaultResults: DiscoveryResult[] = [
  {
    id: "res-001",
...
export class DiscoveryService {
  public static async start(request: DiscoveryRequest) {
    ...
    const response = new ApiResponse<DiscoveryScan>(scan);
    response.addSuccess();
    return response;
  }

  public static async getResults(scanId: string) {
    const stored = scanStore.get(scanId);
    if (stored) {
      const response = new ApiResponse<DiscoveryResult[]>(stored.results);
      response.addSuccess();
      return response;
    }

    const response = new ApiResponse<DiscoveryResult[]>(defaultResults);
    response.addSuccess();
    return response;
  }

  public static async getPlaceDetails(placeId: string) {
    ...
    const params = new URLSearchParams({
      place_id: placeId,
      fields:
        "place_id,name,formatted_address,international_phone_number,formatted_phone_number,opening_hours,website,rating,user_ratings_total,types,business_status,geometry",
      key: apiKey
    });
    ...
    const details: DiscoveryPlaceDetails = {
  ...
  private static resolveTypes(categories: string[]) {
  ...
  private static async fetchGooglePlaces(request: DiscoveryRequest): Promise<DiscoveryResult[]> {
    const apiKey = process.env.GOOGLE_MAPS_API_KEY;
    if (!apiKey) {
      throw new Error("Missing server-side Google Maps API key (GOOGLE_MAPS_API_KEY).");
    }
  ...
  private static extractAddress(place: GooglePlace): string {
    const formatted =
